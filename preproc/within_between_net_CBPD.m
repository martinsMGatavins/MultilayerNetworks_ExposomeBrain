...
myqavars=readtable(strcat('~/Desktop/exposome_project/rsfmri_preprocessing/n198_within_between_Yeo7_', parcellation, '_', pipeline, '_QA_revisions.csv'),'Delimiter',',','ReadVariableNames', 1)        
%running with the cluster mounted locally
datadir=strcat('~/Desktop/Cluster/CBPD_bids_crosssectional/derivatives/xcpEngine_',pipeline)
z_outdir=fullfile('~/Desktop/exposome_project/rsfmri_data/',pipeline,strcat(parcellation,'zNetworks'))
noz_outdir=fullfile('~/Desktop/exposome_project/rsfmri_data/',pipeline,strcat(parcellation,'Networks'))
...
%% Z-score FC matrices
for n=1:height(myqavars)
    sub=char(myqavars.ID(n)) %look at this
    run=char(myqavars.run(n))
...
datadir=strcat('~/Desktop/exposome_project/rsfmri_data/',pipeline,'/',parcellation,'zNetworks')
z_avg_outdir=fullfile('~/Desktop/exposome_project/rsfmri_data/',pipeline,strcat(parcellation,'zNetworks_avg'))
...
% insert value once known
myqavars=readtable(strcat('~/Desktop/exposome_project/rsfmri_preprocessing/n198_within_between_Yeo7_', parcellation, '_', pipeline, '_QA_revisions.csv'),'Delimiter',',','ReadVariableNames', 1)        
%myqavars=readtable(strcat('/Users/utooley/cbica/home/tooleyu/projects/in_progress/within_between_network_conn_CBPD/data/imageData/',pipeline,'/n198_within_between_Yeo7_', parcellation, '_', pipeline, '_QA_revisions.csv'),'Delimiter',',','ReadVariableNames', 1)
dim=str2double(parcellation(end-2:end))%what is the dimensionality of the parcellation
[unique_subs, index]=unique(myqavars.ID)
unique_subjlist= myqavars(index,:)
...
clear modul_comms
datadir=strcat('~/Desktop/exposome_project/rsfmri_data/',pipeline,'/',parcellation,'zNetworks_avg')
% check the folders where community affilian vector files are (.1D)
yeo_nodes=dlmread(fullfile('~/Desktop/Cluster/tools/',parcellation,strcat(parcellation,'x7CommunityAffiliation.1D.txt')))
...
between_system_connectivity=zeros(height(unique_subjlist),7);
...
for i = 1:length(nCi) % loop through communities
    for j = 1:length(nCi)
       ...
       between_system_connectivity(n,i)=mean(Btw(Btw~=0));
...
outdir=strcat('~/Desktop/exposome_project/rsfmri_data/',pipeline)
save(fullfile(outdir, strcat(sub, 'modul_communities_fd0.5_pipeline_',parcellation,'.mat')), 'modul_comms');
...
outdir=strcat('~/Desktop/exposome_project/rsfmri_data/',pipeline)
header={'ID', 'avgweight', 'modul_avg', 'avgclustco_both','num_comms_modul_avg','part_coef_pos','part_coef_neg', 'system_segreg', 'mean_within_sys', 'mean_between_sys', 'sys1to1','sys1to2','sys1to3','sys1to4','sys1to5','sys1to6','sys1to7','sys2to1','sys2to2','sys2to3','sys2to4','sys2to5','sys2to6','sys2to7','sys3to1','sys3to2','sys3to3','sys3to4','sys3to5','sys3to6','sys3to7','sys4to1','sys4to2','sys4to3','sys4to4','sys4to5','sys4to6','sys4to7','sys5to1','sys5to2','sys5to3','sys5to4','sys5to5','sys5to6','sys5to7','sys6to1','sys6to2','sys6to3','sys6to4','sys6to5','sys6to6','sys6to7','sys7to1','sys7to2','sys7to3','sys7to4','sys7to5','sys7to6','sys7to7','intersys1','intersys2','intersys3','intersys4','intersys5','intersys6','intersys7'}

outfile=table(char(unique_subjlist.ID), avgweight, modul, avgclustco_both, num_comms_modul, part_coef_pos, part_coef_neg, system_segreg, mean_within_sys, mean_between_sys, system_conn, between_system_connectivity)
outfile2=splitvars(outfile, 'system_conn')
outfile3=splitvars(outfile2, 'between_system_connectivity')
outfile3.Properties.VariableNames=header

% add number of subjects
save(fullfile(outdir, strcat('n198_long_inc_within_between_Yeo7_avgruns_',parcellation,'_withmodulpartcoef.mat')), 'outfile3')
writetable(outfile3,fullfile(outdir,strcat('n198_long_inc_within_between_Yeo7_avgruns_',parcellation,'_withmodulpartcoef.csv')))
...
save(fullfile(outdir, strcat('n198_long_inc_mean_system_conn_avgruns_',parcellation,'.mat')), 'mean_system_conn_mat')
...
export(outfile,'File',strcat(outdir,'/n198_long_inc_part_coef_avg_nodewise_avgruns', parcellation,'.csv'),'Delimiter',',')
