...
structural_crime <- functional_data[, crime_vars]
functional_crime <- functional_data[, crime_vars]
...
descriptives(data = structural_crime,
             sd = T, #Lists standard deviations
             variance = T, #Lists variances 
             ci = T, #Lists confidence intervals (default: 95%)
             boxLabelOutliers = T, #Lists rows with outliers
             hist = T) #Creates histograms for variables
```
...
# Exposome-structural measure sample (N=167)
## Model fitting
### Parallel one-factor
```{r setup, include=FALSE}
...
### Congeneric two-factor
```{r setup, include=FALSE}
structural_congeneric_twofactor <- sem(model = congeneric_onefactor,
                                       data = structural_crime,
                                       missing = 'fiml',
                                       estimator = 'MLR')
...
# Exposome-functional measure sample (N=131)
## Model fitting
### Parallel one-factor
```{r setup, include=FALSE}
...
### Congeneric two-factor
```{r setup, include=FALSE}
functional_congeneric_twofactor <- sem(model = congeneric_onefactor,
                                       data = functional_crime,
                                       missing = 'fiml',
                                       estimator = 'MLR')
